...
def normalize_patterns(X):
    epsilon = 1e-8
    print('Normalization: Min values:', X.min(axis=0))
    print('Normalization: Max values:', X.max(axis=0))
    print('Normalization result:', (X - X.min(axis=0)) / (X.max(axis=0) - X.min(axis=0) + epsilon))
...
def linear_associator_learning(X, Y):
    ...
    for i in range(len(X)):
        print(f'Outer product of Y[{i}] and X[{i}]:', np.outer(Y[i], X[i]))
        M += np.outer(Y[i], X[i])
        print(f'M matrix after updating with Y[{i}] and X[{i}]:', M)
    return M

# Fase de recuperación del CHAT usando similitud máxima
def recover_class_vectorized(M, X):
    similarities = M.dot(X.T)
    recovered_classes = np.argmax(similarities, axis=0)
